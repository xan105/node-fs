...
async function ls(dirPath, option = {}) {
  ...
  const options = {
    ...
    ignore: {
      ...
      symlink: asBoolean(option.ignore?.symlink) ?? true,
  ...
  { 
    ...
    if (file.isDirectory()){
        ...
        if (subdir.length > 0) {
            if(!options.absolute){
                result = result.concat(subdir.map((el) => {
                    const path = join(file.name, el.path || el);
      ...
      if (options.ext.length > 0 && !options.ext.includes(ext)) continue;
        ...
        ([link] = await attempt(fs.readlink,[join(dirPath, file.name)]));
    ...
    const path = options.absolute ? resolve(join(dirPath, file.name)) : file.name;
    result.push(link ? { path, link } : path);   
  }
