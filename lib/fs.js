  ...
  try{
    ...
    if (err.code === 'ENOTEMPTY') {
      ...
      for (const file of files) 
      {
        ...
        file.isDirectory() ? await rmdir(filePath) : await fs.unlink(filePath);     
      }
      await rmdir(dirPath);        
    }
  }
...
async function ls(dirPath, option = {}) {
  ...
  try{
    ...
    for (const file of files) 
    {
      ...
      else if (file.isFile() && options.excludeFile) continue;
      result.push(options.absolute ? join(dirPath, file.name) : file.name);   
    }
  }catch{ /*Do nothing*/ }
